...
\section{个人简介}
\cvitem{}{2026 届 CS 本科毕业，专注 AI Native 全栈工程——不只使用 AI 工具，更构建 AI Agent 可调用的工具、运行时与决策规范。现就职于雷石（KTV 行业龙头）AI 团队（实习转正），主力开发 AI 客服 Agent 平台与 ERP 三端业务，另主导过跨境短剧 AIGC 系统技术架构。}
...
\section{工作经历}
\cventry{2026.01--至今}{AI 应用开发工程师（实习转正）}{北京雷石天地电子技术股份有限公司 / AI 团队}{北京}{}{
\begin{itemize}
    \item 主力开发企业级 AI 客服 Agent 平台「小雷」（TypeScript / Node.js + Fastify）：微信 + 飞书双渠道消息网关（RabbitMQ + WebSocket 长连接），Orchestrator 调度可插拔多 Agent 后端（Claude Code / Kimi / OpenCode SDK），通过 MCP 挂载业务工具，配套知识库系统与按群聊作用域的知识隔离；已投产承接 VOD 产品线技术支持咨询。
    \item 从 0 到 1 交付 VOD 自动退款 Agent（微信消息采集 → OCR 提取订单号 → CMS 接口对接 → 飞书告警），已投产替代人工退款流程；工程化踩坑沉淀：并发锁防双倍退款、已退款订单幂等判断、异常容错重试。
    \item 负责客用小程序（uni-app + Vue3）、掌柜端管理后台（Vue3）和 Go 后端服务三端开发，交付自助预约、扫码开台、卡券核销、超时自动关台等完整业务闭环；攻克跨端数据一致性问题（预约时段冲突、session 状态漂移），抽象为状态机模型在领域服务层统一修复。
    \item 深入 Go DDD 四层架构后端（规则引擎 / 流程引擎），承担订单、预约、会员等核心交易链路的常规迭代（270+ commits），配套单元测试与集成测试。
    \item 为客用端 / 掌柜端建立 Playwright e2e 自动化验收体系：真实 H5 UI 驱动 + MySQL 状态断言，覆盖预约、开台、会员专属价等核心链路，作为发版闸门替代人工回归；日常开发全面接入 Claude Code，以 CLAUDE.md 规范沉淀代码库上下文与团队踩坑经验。
\end{itemize}}
...
\section{专利、竞赛与荣誉}
\cvitem{2026}{发明专利申请（已受理，审查中）：一种基于常驻音频流的低延迟语音输入方法及系统}
